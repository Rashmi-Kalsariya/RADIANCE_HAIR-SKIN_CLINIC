	...
	function initPreLoader() {
	    ...
	    if (performance.navigation.type === performance.navigation.TYPE_RELOAD ||
	        ...
	        jQuery(window).on('load', function() {
	            setTimeout(hideLoader, 800); // Small delay to show loader briefly
	        });

	        // Fallback - force hide loader after maximum 3 seconds
	        setTimeout(function() {
	            if (jQuery('#loader').length && !loaderHidden) {
	                hideLoader();
	            }
	        }, 3000);
	    } else {
	        // Hide loader immediately if it's not a fresh page load
	        jQuery('#loader').hide().remove();
	    }
	}
